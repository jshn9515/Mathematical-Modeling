...
Mdl = OutputCodeClassifier(KNeighborsClassifier(n_neighbors=20, n_jobs=16), n_jobs=16)
t1 = time.time()
Mdl.fit(train, label)
t2 = time.time()
...
train = df1.iloc[:, :-1].values
label = df1.iloc[:, -1].values
Mdl = OutputCodeClassifier(KNeighborsClassifier(n_neighbors=5, n_jobs=16), n_jobs=16)
t1 = time.time()
Mdl.fit(train, label)
t2 = time.time()
print(f'Training time: {t2 - t1:.2f}s')
